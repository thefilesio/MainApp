...
export const usePrompts = (botId?: string) => {
  ...
  const createPrompt = async (input: CreatePromptInput): Promise<Prompt> => {
    ...
    await createVersionSnapshot(input.bot_id, input.step, input.content ?? "");
    return data;
  };
  ...
  const updatePrompt = async (id: string, content: string | null): Promise<Prompt> => {
    ...
    await createVersionSnapshot(prompt.bot_id, prompt.step, content ?? "");
    return data;
  };

  const createVersionSnapshot = async (botId: string, step: number, content: string) => {
    if (!user) return;
    const { error } = await supabase
      .from("versions")
      .insert({ bot_id: botId, prompt_snapshot: { step, content } });
