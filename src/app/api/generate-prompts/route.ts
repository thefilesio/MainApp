...
export async function POST(req: Request) {
  try {
    ...
    if (!companyInfo) {
      return new Response(
        JSON.stringify({ error: 'Company info is required' }),
        { status: 400 }
      );
    }
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a helpful assistant that generates relevant questions for a chatbot based on company information.
        ...
        {
          role: 'user',
          content: `Generate questions based on this company information:\n\n${companyInfo}`
        }
      ],
