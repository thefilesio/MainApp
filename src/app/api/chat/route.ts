...
const supabase = createClient(
  process.env.SUPABASE_URL!, // nicht NEXT_PUBLIC_, da Server-only
...
export async function POST(req: Request) {
  try {
    ...
    const chatMessages = [
      { role: 'system', content: bot.preset_prompt || 'You are a helpful assistant.' },
      ...messages,
    ];
    ...
    try {
      await supabase.from('messages').insert([
        { bot_id: botId, user_id: bot.user_id, role: 'user', content: lastUserMessage },
        { bot_id: botId, user_id: bot.user_id, role: 'assistant', content: assistantMessage.content },
      ]);
    } catch (logError) {
